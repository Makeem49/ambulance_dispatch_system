...
class AuthOperations:
    ...
    def login(data):
        ...
        if (
            hasattr(user, "totp_auth")
            and user.totp_auth.otp_enabled
            and user.totp_auth.otp_verified
        ):
    ...
    def logout(data):
        ...
        try:
            ...
            Token.objects.filter(refresh_token=data.get("refresh_token")).update(
                is_blacklisted=True
            )
            ...
            if request:
                ...
                if hasattr(user, "totp_auth"):
                    ...
                    if not token:
                        return (
                            None,
                            "OTP is required to change user with MFA activated",
                            status.HTTP_400_BAD_REQUEST,
                        )

                    data = {"token": token, "user": user}

                    data, error, status_code = AuthOperations.verify_otp(
                        data
                    )  # verify totp token
                    if error:
